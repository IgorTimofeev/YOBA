...
namespace YOBA {
	void UTF8::nextCodepoint(const std::string_view str, size_t& index, uint32_t& codepoint) {
		...
		if ((byte1 & 0b1110'0000) == 0b1100'0000) {
			...
			index += 2;
			codepoint = ((byte1 & 0b0001'1111) << 6) | (byte2 & 0b0011'1111);
		...
		if ((byte1 & 0b1111'0000) == 0b1110'0000) {
			...
			index += 3;
			codepoint = ((byte1 & 0b0000'1111) << 12) | ((byte2 & 0b0011'1111) << 6) | (byte3 & 0b0011'1111);
		...
		if ((byte1 & 0b1111'1000) == 0b1111'0000) {
			...
			index += 4;
			codepoint = ((byte1 & 0b0000'0111) << 18) | ((byte2 & 0b0011'1111) << 12) | ((byte3 & 0b0011'1111) << 6) | (byte4 & 0b0011'1111);
